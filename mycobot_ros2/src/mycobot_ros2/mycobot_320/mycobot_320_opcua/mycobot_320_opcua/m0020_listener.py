...
# M0020 센서 상태를 수신하고 메시지를 출력하는 노드
class SensorListener(Node):
    def __init__(self):
        super().__init__('sensor_listener')
        self.get_logger().info('M0020 센서 리스너 노드 초기화. plc/sensor_status 토픽 구독 시작.')
    ...
    def listener_callback(self, msg):
        ...
        # M0020 값이 1일 때 (센서가 활성화되었을 때) "Hello~" 출력
        if sensor_value == 1:
            self.get_logger().info('M0020 센서 활성화 감지: Hello~')
        # 값이 0일 때 (선택 사항: 센서 비활성화 시 메시지)
        elif sensor_value == 0:
            # self.get_logger().info('M0020 센서 비활성화.')
            pass  # 0일 때는 아무것도 출력하지 않도록 설정
        else:
            self.get_logger().warn(f'M0020에서 예상치 못한 값 수신: {sensor_value}')
