...
class ROS_Robot:
    ...
    def gripper_open(self):
        try:
            ...
            self.result.append(["grasp", [self.gr_open, self.gr_speed]])
        except Exception as e:
    ...
    def gripper_close(self):
        try:
            ...
            self.result.append(["grasp", [self.gr_close, self.gr_speed]])
        except Exception as e:
    ...
    def place_box(self, val: str, idx: int=0):
        ...
        try:
            if val in ('green', 'normal'):
                z = z + 148
                # 접근 자세(관절)
                self.move_and_wait("angles", [-10.0, 0.0, 78.95, -21.0, -87.36, -15.0])
                # 드롭 좌표(툴 좌표)
                ...
                self.gripper_open()
                # 이탈 자세
                self.move_and_wait("angles", [-22.23, 0.0, 78.95, -21, -87.36, -15.0]) 

            elif val == 'blue':
                z = z + 170
                self.move_and_wait("angles", [12.12, 0.0, 70.83, -16.08, -67.5, -150.0])
                self.move_and_wait("coords", [-219.2, -281.9, z, 179.41, -5.0, -100.0], speed=10, delay=1.0)
                # self.mc.set_gripper_value(40, 20, 1)
                self.gripper_open()
                self.move_and_wait("angles", [12.12, 0.0, 70.83, -16.08, -67.5, -150.0])
                ...
                z = z + 160
                self.move_and_wait("angles", [136.66, 0.0, -55.98, 0.0, 109.51, -30.0])
                self.move_and_wait("coords", [-180.0, 240.0, z, -173.15, 0.0, 90.0], speed=10, delay=1.0)
                # self.mc.set_gripper_value(40, 20, 1)
                self.gripper_open()
                self.move_and_wait("angles", [136.66, 0.0, -55.98, 0.0, 109.51, -30.0])

            elif val in ('anomaly'):
                self.move_and_wait("angles", [-80.0, -20.0, -65.0, -0.0, 90.0, -90.0])
                # self.mc.set_gripper_value(40, 20, 1)
                self.gripper_open()
    ...
    def go_anchor(self, speed=20, delay=0.2):
        try:
            ...
            """앵커 좌표로 복귀"""
            self.move_and_wait("angles", [0.0, 0.0, -80.0, -0.0, 90.0, -90.0], speed, delay)
        except Exception as e:
