...
class DetectorNode(Node):
    def __init__(self):
        ...
        if self.mode == "detect_and_classify":
            ...
            self.get_logger().info(f"[PATH] YAML  : {self.yaml_path}")
            self.get_logger().info(f"[PATH] WEIGHT: {self.model_path}")
            self._init_yolo()
    ...
    def loop(self):
        ...
        msg.data = [float(x_t), float(y_t), float(rz_t if rz_t else 0.0),
                    color_map.get(selected["color"], 0.0), -1.0]
        ...
        if self.mode == "detect_and_classify":
            ...
            cv2.putText(overlay, f"STATE: {state_txt}", (self.W - 220, 30),
