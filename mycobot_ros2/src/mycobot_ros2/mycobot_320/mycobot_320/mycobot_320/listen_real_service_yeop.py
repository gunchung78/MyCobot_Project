...
class MyCobotDriver(Node):
    ...
    def __init__(self):
        ...
        self.group_pub = ReentrantCallbackGroup()
        self.group_srv = ReentrantCallbackGroup()
        ...
        self.timer = self.create_timer(0.1, self.publish_joint_states, callback_group=self.group_pub)

        # Service servers
        self.srv_angles = self.create_service(SetAngles, 'set_angles', self.set_angles_callback, callback_group=self.group_srv)
        self.srv_coords = self.create_service(SetCoords, 'set_coords', self.set_coords_callback, callback_group=self.group_srv)
        self.srv_get_coords = self.create_service(GetCoords, 'get_coords', self.get_coords_callback, callback_group=self.group_srv)
        self.srv_get_angles = self.create_service(GetAngles, 'get_angles', self.get_angles_callback, callback_group=self.group_srv)
        self.srv_gripper = self.create_service(GripperStatus, 'set_gripper', self.set_gripper_callback, callback_group=self.group_srv)
        self.srv_pump = self.create_service(PumpStatus, 'set_pump_status', self.set_pump_callback, callback_group=self.group_srv)
        self.srv_force_gripper = self.create_service(GripperStatus, 'set_force_gripper', self.set_force_gripper_callback, callback_group=self.group_srv)

    def publish_joint_states(self):
        """Publish joint_states + (angles/coords) at 10 Hz with independent locks."""
        try:
            # ---- get_angles ----
            angles = None
            for _ in range(3):
                fd = acquire('/tmp/mycobot_lock')
                if fd is None:
                    self.get_logger().warn("get_angles: lock busy, retry...")
                    time.sleep(0.02)
                    continue
                a = self.mc.get_angles()
                release(fd)
                if isinstance(a, list) and len(a) == 6 and all(x != -1 for x in a):
                    angles = a
                    break
                time.sleep(0.05)
            ...
            if angles is None:
                if self._last_valid_angles:
                    ...
                    self.get_logger().warn("get_angles invalid, using last valid cache.")
                else:
                    return
            else:
            ...
            for _ in range(3):
                ...
                release(fd)
                if isinstance(c, list) and len(c) == 6 and all(x != -1 for x in c):
                    coords = c
                    break
                time.sleep(0.05)

            # ---- Publish JointState (rad) ----
            js = JointState()
            js.header = Header()
            ...
            # ---- Publish Coords ----
            if coords:
                msg_coords = Float32MultiArray()
                msg_coords.data = [float(c) for c in coords]
                self.pub_coords_f32.publish(msg_coords)
            else:
                self.get_logger().warn("coords invalid, skipped publish.")

        except Exception:
            self.get_logger().error(f"publish_joint_states error:\n{traceback.format_exc()}")
    ...
    def get_coords_callback(self, request, response):
        ...
        try:
            lock = acquire('/tmp/mycobot_lock')
            if lock is None:
                self.get_logger().warn("get_coords: read lock busy, skip")
                return response
